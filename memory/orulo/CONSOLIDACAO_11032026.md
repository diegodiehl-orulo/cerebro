...
2. Planilha `WS_OPERATING_SYSTEM_H1_2026` — cockpit operacional
...
- `98_BASES_DIEGO` — materiais de apoio técnico
- `99_BACKUP` — staging e backup
...
```bash
gog-morfeu drive get <id_do_arquivo_ou_pasta> --json | jq '.file.parents[0]'
```
...
| Pasta | Documento |
|-------|-----------|
| `00_README_GERAL` | README_GERAL |
| `01_GOVERNANCA_GERAL` | ARQUITETURA_OFICIAL_V1 |
| `01_GOVERNANCA_GERAL` | CADENCIA_OPERACIONAL |
| `01_GOVERNANCA_GERAL` | MAPA_DE_RESPONSAVEIS_V1 |
| `01_GOVERNANCA_GERAL` | MAPA_ESTRUTURA_DRIVE_SISTEMA_OPERACIONAL |
| `01_GOVERNANCA_GERAL` | WS_OPERATING_SYSTEM_H1_2026 *(planilha)* |
| `05_TEMPLATES` | TEMPLATE_WS_REUNIAO |
...
- `WSX_ONEPAGE_CHARTER` — referência estratégica e escopo
- `WSX_REUNIOES` — registro de reuniões
- `WSX_NOTAS` — notas de trabalho
...
O `MAPA_DE_RESPONSAVEIS_V1` é a fonte oficial para:
- sponsor
- DRI
...
**Localização:** `01_GOVERNANCA_GERAL` → `1VQDXQS6otxWsHKRuAZAXks0ps8Q9JJ5d`

## 7.1 Papel da planilha

A planilha `WS_OPERATING_SYSTEM_H1_2026` é o cockpit operacional do sistema.
...
| 2 | **Praça** | Geral ou praça específica (ex: Curitiba_PR) |
...
```
BACKLOG → PLANO_DE_ACAO → STATUS_QUINZENAL → EVIDENCIAS → DECISOES
```
...
## 8.1 Papel dos ONEPAGE_CHARTERS

Os ONEPAGE_CHARTERS são a memória estrutural oficial de cada workstream. Eles definem:
- função estratégica
- objetivo
- escopo
- fora de escopo
...
| Charters | ✅ 7 WS com ONEPAGE_CHARTER |
...
| BACKLOG_GERAL | ⚠️ 70 itens — sem prazos — alguns sem Frente/Evidência |
| PLANO_DE_ACAO | 🔴 Estrutura pronta, sem dados |
| STATUS_QUINZENAL | 🔴 Estrutura pronta, sem dados |
| EVIDENCIAS | 🔴 Estrutura pronta, sem dados |
| DECISOES | 🔴 Estrutura pronta, sem dados |
| DRIS_E_RESPONSAVEIS | ✅ Preenchido (divergências residuais) |
...
- 🟡 **Ciclo evidência não testado** — EVIDENCIAS e STATUS_QUINZENAL vazios
...
4. Corrigir PORTFOLIO e DRIS_E_RESPONSAVEIS conforme MAPA
